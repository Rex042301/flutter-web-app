...
class EWSCard extends StatefulWidget {
  final bool isAdmin;
  final VoidCallback? onTap;

  const EWSCard({super.key, required this.isAdmin, this.onTap});

  @override
  State<EWSCard> createState() => _EWSCardState();
}

class _EWSCardState extends State<EWSCard> {
        ...
        child: Container(
          ...
          child: Column(
            children: [
              Flexible(
                flex: 3,
                child: Icon(Icons.notifications_active_rounded, size: height * 0.3, color: Colors.blueAccent),
              ),
              Flexible(
                flex: 2,
                child: FittedBox(
